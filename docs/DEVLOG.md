...
Closed out the desktop track with the user-facing docs and a fresh installer that
folds in everything since 1.0.0 (citation engine v2, DOI auto-fill, atomic ingest,
the quotation-mark icon, and the pg_ctl console-window-flash fix).

- **README rewritten for "download and run".** Leads with the Windows installer
  path (per-user, SmartScreen "Run anyway" note, what is bundled vs. the runtime
  LLM choice, system requirements), then keeps a "run from source" section for
  developers (Docker quickstart, Python API, evaluation, and how to build the
  installer). Added a brand banner at the top (`docs/banner.png`, generated by
  `make_banner.py`) and dropped em dashes per the house style.
- **Version bumped to 1.1.0** (`CiteFinder.iss`); same AppId, so it installs as an
  in-place upgrade over 1.0.0.
- **Clean rebuild**: wiped `dist/` and `build/`, regenerated the icon, ran
  PyInstaller (`CiteFinder.spec`) and Inno Setup (`CiteFinder.iss`) to produce
  `installer/CiteFinder-Setup-1.1.0.exe`.
- Screenshots/demo capture was attempted via a headless browser against the
  running app but the headless-shell download failed on this machine; left as a
  manual step (the banner is in place).

### 2. Tests

**T43 - clean 1.1.0 build**
- Wiped `dist/` + `build/`, ran PyInstaller (`--clean`, exit 0). Onedir is 1.2 GB:
  launcher `CiteFinder.exe` 25.7 MB (icon embedded - PyInstaller "Copying icon to
  EXE"), and the bundle carries the e5 model (`model/model.onnx` 133 MB +
  `tokenizer.json`), the SPA (`web/`), `citefinder.ico`, and the portable Postgres
  + pgvector (`pgsql/bin/postgres.exe`, `pgsql/lib/vector.dll`) - all verified
  present.
- Inno Setup compiled in ~9 min (exit 0) -> `installer/CiteFinder-Setup-1.1.0.exe`,
  **332 MB**.
- The window render is interactive (opens a native WebView2 window), so it is
  smoke-tested by installing and launching, not in this headless pass; the
  server-path + bundled-Postgres wiring is the same code verified in T35/T36.
