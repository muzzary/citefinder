...
- CrossRef REST API (opt-in DOI → bibliographic-metadata lookup for the cite flow; stdlib `urllib`, no key, only the DOI string leaves the machine — see `crossref.py`)
...
INGEST: PDF -> extract text per page -> chunk + tag with page/source/user -> embed -> store source + chunks in pgvector **atomically** (one transaction, so a half-ingested source is never visible and a delete racing the ingest can't orphan it). Metadata (title/author/year guess + any DOI on the first pages) is auto-extracted; confirmation is optional and deferred, so the student can query immediately.
...
README.md, AGENTS.md # stay at root by convention
```
...
- **setup_db.py** — Creates the database tables (`sources`, `chunks`, `chats`, `messages`) and runs idempotent migrations. Schema: sources hold attribution metadata plus `kind` (work | notes), `confirmed`, `chat_id` (which chat owns this source — see [ADR 0005](docs/0005-chat-owns-its-corpus.md)), and the citation-typing columns `work_type` (book | article | website) + `cite_meta` (JSONB, the per-type fields) + `doi` (auto-detected, the CrossRef lookup key); chunks hold text + page + user_id + the embedding vector + a generated `tsvector` (`text_tsv`) with a GIN index for the keyword half of hybrid search; chats own a corpus; messages hold each chat's Q&A turns (with `attribution` JSONB). Safe to re-run.
...
- **embedder.py** — The local embedding model (`e5-small-v2` via ONNX Runtime). `embed(texts, kind="query"|"passage")` applies the e5 asymmetric prefix; model files resolve from a bundled dir / `CITEFINDER_MODEL_DIR` / the HF cache (`local_files_only` first, so a cache hit needs no network). `warmup()` pre-loads the session (app.py runs it in a background thread at boot to hide the cold start).

- **embed_store.py** — `store_source_and_chunks` is the atomic ingest path: embed all chunks (slow, outside any transaction), then insert the source row + its chunks in ONE transaction so the source only becomes visible once complete (closes the delete-during-ingest race). `store_source` / `store_chunks` remain for the eval/benchmark scripts.

- **metadata.py** — `extract_metadata` auto-extracts author/title/year from a PDF's embedded metadata (a guess to confirm — often unreliable; the embedded "year" is the file's creation date). `extract_doi` finds the first DOI in some text (the PDF's first pages), the key for the CrossRef cite lookup.

- **crossref.py** — Opt-in DOI → authoritative bibliographic metadata via the CrossRef REST API (`lookup_doi`, behind the "Auto-fill from DOI" button). Maps CrossRef's record onto the confirm-form schema (work_type + author/title/year + type fields); the student still confirms it. stdlib `urllib`, no key; only the DOI string leaves the machine (ADR 0002).
...
- **citations.py** — Pure formatting (no DB, no LLM): `format_locator` builds the always-honest Locator (file + page + summary + context). `format_citation(meta, style)` is **work-type aware** (`book` | `article` | `website`) and returns **both** forms — `{in_text, reference}` — for APA / Harvard / IEEE (9 shapes), built from real fields (author/title/year + the type's cite_meta). In-text omits the page; titles render verbatim; blank optional fields are dropped. Per [ADR 0003](docs/0003-locate-by-default-cite-on-confirmation.md) the Citation is a confirmed-only extra, the Locator the default.

- **sources.py** — Source lifecycle (the "cite this source" backend; parallel to chats.py). `list_sources_for_chat(chat_id)` returns the files in a chat (filename/title, kind, confirmed, work_type, cite_meta, doi, chunk count); `get_source` returns a source row as a dict (the UI checks `confirmed`); `confirm_source(source_id, author, title, year, work_type, meta)` stores real metadata + the type's fields (kept only if valid for the type) and **locks** the Source as confirmed (persisted; needs author + year + a real title or it stays unconfirmed; Notes can't be confirmed); `cite_source(source_id, style)` renders the `{in_text, reference}` Citation for a confirmed Source, with **style chosen at cite time, never stored**; `delete_source(source_id)` removes one Source + its chunks (cascade). See [ADR 0003](docs/0003-locate-by-default-cite-on-confirmation.md).
...
- **app.py** — The Phase-8 web server (FastAPI): a thin JSON layer over the pipeline with no logic of its own. Serves the `web/` SPA and exposes chats (create / list / rename / **delete** / messages / **sources**), `upload` (multipart → `ingest_pdf` per PDF, non-blocking; auto-extracts the metadata guess, skips a PDF whose name is already in the chat), `ask` (→ `query.answer`, builds the attribution list, stores both turns — runs `answer` first and returns a clean 503 if the local model is down), cite (`get_source` / `confirm_source` / `cite_source`), **`delete` a single source** (`DELETE /api/sources/{id}` → source + chunks + the PDF on disk), and **`lookup-doi`** (`POST /api/lookup-doi` → `crossref.lookup_doi` for the auto-fill button). Pre-warms the embedder in a background thread at boot. Ingest/ask are sync `def` so FastAPI runs them in a threadpool. `user_id` is fixed to `user_1` (single-user v1). Run `python app.py`, open http://localhost:8000.

- **web/** — The browser UI (vanilla SPA, no build step — see the Phase-8 DEVLOG for why not React). `index.html` (SVG icon library + fonts), `styles.css` (near-black theme, one emerald accent, Space Grotesk / Outfit / JetBrains Mono), `app.js` (hash router: `#/` home with the two start options, `#/chats` list, `#/chat/:id` chat with the previous-chats sidebar; file + folder upload; the Files menu with per-file delete; the full cite flow — confirm-state check → confirm form (work-type picker + per-type fields + DOI auto-fill) → style picker → rendered in-text + reference; toasts; loading/empty/error states).
...
- **make_icon.py** — Build-time tool (not a runtime dep) that generates `citefinder.ico` — a bold quotation mark (the citation symbol) in the dark accent-foreground on the emerald accent tile. Used by `desktop.py` (runtime window icon) and `CiteFinder.spec` (exe/installer icon).
...
- **docs/0001–0007 \*.md** — Architecture Decision Records.
